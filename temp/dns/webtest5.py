...
DNS_QUERY_TYPE = 'TXT'
USE_DNSSEC = True
DEST_PORT = 8080               # 保持8080端口
DNS_IP_LIST_FILE = "niceip.txt"
QUERY_NAME = "ripe.net"
TARGET_PPS = 10000             # 目标每秒包数
THREAD_COUNT = 8               # 恢复为8个线程
...
def build_raw_packet(dst_ip):
    ...
    # UDP头部 - 关键修复：使用固定源端口DEST_PORT，确保响应能返回
    udp_src = DEST_PORT  # 使用固定端口，确保DNS响应返回到正确端口
    udp_dst = 53
...
def print_rate_dashboard():
    ...
    print("\n" + "="*80)
    print(f"{status_emoji} DNS压力测试实时监控面板 {status_emoji}")
    print("="*80)
    ...
    print(f"{resp_emoji} DNS响应率: {response_rate:>6.1f}% ({recv_current_pps:.0f}/{send_current_pps:.0f})")
    ...
    # 线程状态
    active_threads = threading.active_count() - 3  # 减去主线程、统计线程和接收线程
    print(f"🧵 活动线程: {active_threads}/{THREAD_COUNT}")
...
def recv_worker():
    ...
    try:
        ...
        # 增加接收缓冲区大小
        recv_sock.setsockopt(socket.SOL_SOCKET, socket.SO_RCVBUF, 1024 * 1024)  # 1MB缓冲区
        recv_sock.bind(('0.0.0.0', DEST_PORT))
        recv_sock.settimeout(0.1)  # 设置短超时以便检查停止条件
        print(f"📡 接收线程启动，监听端口 {DEST_PORT}")
    ...
    while is_running and time.time() < end_time:
        try:
            ...
            if packets_received > 0 and packets_received % 100 == 0 and time.time() - last_log_time > 2:
                print(f"📥 接收调试: 已接收 {packets_received} 个响应包")
                last_log_time = time.time()
...
def send_worker(worker_id):
    ...
    # 简单批次控制，不使用复杂缓存
    batch_size = min(50, target_pps_per_thread // 20)
    if batch_size < 1:
        batch_size = 1
