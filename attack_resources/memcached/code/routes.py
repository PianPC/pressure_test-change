"""
Memcached 攻击资源获取 - Flask Blueprint

路由前缀: /api/memcached-scan/
提供 Memcached 放大率测量扫描的完整 API：
  - IP 资源文件管理
  - 扫描任务创建/停止/查询
  - 日志与产物读取
"""
...
memcached_scan_bp = Blueprint("memcached_scan", __name__, url_prefix="/api/memcached-scan")

logger = logging.getLogger(__name__)

# 路径常量
REPO_ROOT = Path(__file__).resolve().parents[3]  # pressure_test-change/
MEMCACHED_OUTPUT_ROOT = REPO_ROOT / "attack_resources" / "memcached" / "runs" / "memcached_scan"
MEMCACHED_RESOURCES_ROOT = REPO_ROOT / "attack_resources" / "memcached" / "resources"
...
class MemcachedScanRegistry:
    def __init__(self):
        self.lock = Lock()
        self.scanners: Dict[str, MemcachedResourceScanner] = {}
        self.threads: Dict[str, Thread] = {}
        self.errors: Dict[str, str] = {}
        self.configs: Dict[str, ScanConfig] = {}

    def register(self, run_id: str, scanner: MemcachedResourceScanner, thread: Thread, config: ScanConfig):
        with self.lock:
    ...
    def get_scanner(self, run_id: str) -> Optional[MemcachedResourceScanner]:
        with self.lock:
...
memcached_registry = MemcachedScanRegistry()
...
def _list_ip_files(search_dirs: Optional[List[Path]] = None) -> List[Dict[str, Any]]:
    """????????IP ???????????????????????????????"""
    del search_dirs
    resources = list_protocol_resources("memcached", ATTACK_RESOURCES_ROOT)
    return [_normalize_resource(resource) for resource in resources]


def _resolve_ip_file(value: str) -> Path | None:
    return resolve_protocol_resource_path("memcached", value, ATTACK_RESOURCES_ROOT)
...
def _generate_run_id() -> str:
    return datetime.now().strftime("memcached_%Y%m%d_%H%M%S")
...
def _list_run_dirs() -> List[Dict[str, Any]]:
    """列出历史扫描运行"""
    if not MEMCACHED_OUTPUT_ROOT.exists():
        return []
    runs: List[Dict[str, Any]] = []
    for d in sorted(MEMCACHED_OUTPUT_ROOT.iterdir(), reverse=True):
        if not d.is_dir():
            continue
        run_id = d.name
...
def _read_run_file(run_id: str, filename: str) -> str:
    path = MEMCACHED_OUTPUT_ROOT / run_id / filename
    if not path.exists():
...
def _read_run_log(run_id: str, tail: int = 200) -> str:
    ...
    # 历史运行尝试读日志文件
    log_path = MEMCACHED_OUTPUT_ROOT / run_id / "pipeline.log"
    if log_path.exists():
...
@memcached_scan_bp.route("/resources", methods=["GET"])
def list_ip_resources():
    """列出可用的候选 IP 文件"""
    return jsonify({"success": True, "resources": _list_ip_files()})


@memcached_scan_bp.route("/runs", methods=["GET"])
def list_runs():
    """列出所有扫描运行"""
    runs = _list_run_dirs()
    active = memcached_registry.active_run_ids()
    return jsonify({
        "success": True,
        "runs": runs,
...
@memcached_scan_bp.route("/runs", methods=["POST"])
def start_scan():
    """启动一次 Memcached 资源扫描"""
    ...
    def scan_worker():
        try:
            ...
            try:
                ...
                agg_result = aggregate_quality_ips("memcached", task_qualified_ips_path)
                logger.info(
                    "已聚合 %d 个优质 IP 到 memcached 质量池，总计 %d 个",
                    agg_result.get("added_count", 0),
                    agg_result.get("total_count", 0),
                )
            except Exception as agg_exc:
                logger.error("聚合优质 IP 到 memcached 质量池失败: %s", agg_exc)
...
@memcached_scan_bp.route("/runs/<run_id>", methods=["GET"])
def get_run_detail(run_id: str):
    ...
    if scanner:
        ...
        # 尝试从文件读取历史运行
        stats_path = MEMCACHED_OUTPUT_ROOT / run_id / "final_stats.json"
        summary_path = MEMCACHED_OUTPUT_ROOT / run_id / "scan_summary.json"
        stats = {}
        if stats_path.exists():
            try:
...
@memcached_scan_bp.route("/runs/<run_id>/logs", methods=["GET"])
...
@memcached_scan_bp.route("/runs/<run_id>/stop", methods=["POST"])
def stop_scan(run_id: str):
    """停止扫描"""
    scanner = memcached_registry.get_scanner(run_id)
    if scanner and scanner.is_running:
        scanner.stop()
        return jsonify({"success": True, "message": "正在停止 Memcached 资源扫描 …"})
    return jsonify({"success": False, "message": "没有正在运行的扫描"})


@memcached_scan_bp.route("/runs/<run_id>/results", methods=["GET"])
...
@memcached_scan_bp.route("/runs", methods=["DELETE"])
def clear_runs():
    """清理非活跃的历史运行"""
    import shutil
    ...
    if MEMCACHED_OUTPUT_ROOT.exists():
        for d in sorted(MEMCACHED_OUTPUT_ROOT.iterdir()):
            if not d.is_dir():
                continue
            run_id = d.name
            if run_id in active:
...
@memcached_scan_bp.route("/runs/<run_id>/files/<path:filename>", methods=["GET"])
...
@memcached_scan_bp.route("/state", methods=["GET"])
def scan_state():
    """当前活跃扫描状态"""
    return jsonify({
        "success": True,
...
@memcached_scan_bp.route("/cmd-types", methods=["GET"])
def cmd_types():
    """返回支持的 Memcached 探测命令类型"""
    return jsonify({
        "success": True,
        "types": {name: val for name, val in MEMCACHED_CMD_TYPES.items()},
    })
