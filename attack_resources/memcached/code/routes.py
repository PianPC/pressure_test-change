...
def _build_config(
    ...
    if config.cmd_type not in MEMCACHED_CMD_TYPES:
        return config, (f"不支持的命令类型: {config.cmd_type}", 400)
    return config, None


def _register_extra_routes(bp: Blueprint) -> None:
    @bp.route("/cmd-types", methods=["GET"])
    def cmd_types():
...
_spec = ProtocolScanSpec(
    ...
    blueprint_name="memcached_scan",
    url_prefix="/api/memcached-scan",
    run_id_prefix="memcached",
    output_root_getter=lambda: MEMCACHED_OUTPUT_ROOT,
    attack_resources_root=ATTACK_RESOURCES_ROOT,
    scanner_factory=MemcachedResourceScanner,
    build_config=_build_config,
    config_to_dict=_build_config_dict,
)

memcached_scan_bp, memcached_registry = create_scan_blueprint(
    _spec, _register_extra_routes
)
...
def _list_ip_files(search_dirs: Optional[list] = None):
    del search_dirs
    return _list_ip_files_shared("memcached", ATTACK_RESOURCES_ROOT)


def _resolve_ip_file(value: str):
    return _resolve_ip_file_shared("memcached", ATTACK_RESOURCES_ROOT, value)


def _generate_run_id() -> str:
    return _generate_run_id_shared("memcached")


def _list_run_dirs():
    return _list_run_dirs_shared(MEMCACHED_OUTPUT_ROOT, memcached_registry)


def _read_run_file(run_id: str, filename: str) -> str:
    return _read_run_file_shared(MEMCACHED_OUTPUT_ROOT, run_id, filename)


def _read_run_log(run_id: str, tail: int = 200) -> str:
    return _read_run_log_shared(
        MEMCACHED_OUTPUT_ROOT, memcached_registry, run_id, tail
    )
