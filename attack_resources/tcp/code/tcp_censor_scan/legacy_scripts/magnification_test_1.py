...
METHOD_MAP = {
    ...
    "SYN": 5           # 对应原方法5：SYN with GET（论文方法5）
}
...
def build_packets(ip, ttl):
    ...
    if method == 1:
        ...
        # 方法5：SYN with GET（论文方法5，SYN包携带HTTP GET payload）
        syn_pkt = IP(dst=ip, ttl=ttl)/TCP(dport=80, sport=sport, flags="S", seq=seq)/Raw(load=sensitive_payload)
        packets = [syn_pkt]
    return packets
